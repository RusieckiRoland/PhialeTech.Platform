...
{
    public sealed class DemoLocalizationCatalog
    {
        private readonly Dictionary<string, IReadOnlyDictionary<string, string>> _languages;

        public static DemoLocalizationCatalog Empty { get; } =
            new DemoLocalizationCatalog(new Dictionary<string, IReadOnlyDictionary<string, string>>(StringComparer.OrdinalIgnoreCase));

        public DemoLocalizationCatalog(
        ...
        public IReadOnlyCollection<string> AvailableLanguageCodes => _languages.Keys.OrderBy(code => code, StringComparer.OrdinalIgnoreCase).ToArray();

        public static DemoLocalizationCatalog LoadDefault(string fallbackLanguageCode = "en")
        {
            return LoadFromDirectory(GetDefaultLanguageDirectory(), fallbackLanguageCode);
        }

        public static DemoLocalizationCatalog LoadFromDirectory(string directoryPath, string fallbackLanguageCode = "en")
        {
            ...
            return new DemoLocalizationCatalog(languages, fallbackLanguageCode);
        }

        public static string GetDefaultLanguageDirectory()
        {
            var assemblyDirectory = Path.GetDirectoryName(typeof(DemoLocalizationCatalog).Assembly.Location) ?? AppContext.BaseDirectory;
            var candidates = new[]
            {
                Path.Combine(assemblyDirectory, "PhialeTech.Components.Shared", "Languages"),
        ...
        public string GetText(string languageCode, string key)
        {
            ...
            if (!string.Equals(normalizedLanguageCode, FallbackLanguageCode, StringComparison.OrdinalIgnoreCase) &&
                TryGetText(FallbackLanguageCode, key, out var fallback))
            {
                return fallback;
            }
        ...
        {
            value = null;
            if (_languages.TryGetValue(languageCode, out var language) && language.TryGetValue(key, out var localizedText))
            {
                value = localizedText;
                return true;
            }
            ...
            if (separatorIndex > 0)
            {
                ...
                if (_languages.TryGetValue(neutralLanguageCode, out var neutralLanguage) &&
                    neutralLanguage.TryGetValue(key, out localizedText))
                {
                    value = localizedText;
                    return true;
                }
