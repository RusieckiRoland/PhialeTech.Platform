// PhialeGis.Library.UwpUi.Interactions.Bridges/DslEditorConnector.cs
using System;
    ...
    internal sealed class DslEditorConnector : IDisposable
    {
        ...
        private async Task DebouncedTickAsync()
        {
            try
            {
                ...
                // 1) Semantyka (bez zmian)
                var toks = await _editors.GetSemanticTokensAsync(code, _editor).ConfigureAwait(false);
                SemanticTokensAvailable?.Invoke(this, toks);

                // 2) Heurystyka "piszę token" → wstrzymaj walidację i WYCZYŚĆ poprzednie błędy
                ...
                if (caretInsideToken)
                {
                    // wyczyść stare podkreślenia, żeby nie „wisiała” poprzednia diagnoza
                ...
                // 3) Walidacja (jedna ścieżka: manager → bindings → engine)
