// PhialeGis.Library.WinUi/Bridges/DslEditorConnector.cs
using System;
...
namespace PhialeGis.Library.WinUi.Interactions.Bridges
{
    ...
    internal sealed class DslEditorConnector : IDisposable
    {
        ...
        /// <summary>When action prompt is changing.</summary>
        ...
        private async Task DebouncedTickAsync()
        {
            try
            {
                ...
                // 2) Heuristic "typing a token" → suspend validation and CLEAR previous errors
                ...
                if (caretInsideToken)
                {
                    // clear old underlines so the previous diagnosis does not "hang"
