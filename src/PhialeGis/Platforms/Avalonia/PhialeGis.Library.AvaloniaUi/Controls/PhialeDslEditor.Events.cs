// PhialeDslEditor.Events.cs (Avalonia)
    ...
    public sealed partial class PhialeDslEditor
    {
        internal sealed partial class EditorAdapter : IEditorInteractive
        {
            ...
            internal void RaiseDiagnosticsUpdated(
                ...
                string[] messages)
            {
                DiagnosticsUpdatedUniversal?.Invoke(_owner, new UniversalDiagnosticsUpdatedEventArgs(
            ...
            public void SetText(string text)
            {
                RunOnUI(() =>
                {
                    ...
                    if (!string.Equals(_owner.Text, value, StringComparison.Ordinal))
                        _owner.SetValue(TextProperty, value);

                    _owner.JsEval($"window.__ph_setText({ToJsString(value)})");
                });
            }
            ...
            public void SetLanguageId(string id)
            {
                RunOnUI(() =>
                {
                    var value = id ?? "plaintext";
                    if (!string.Equals(_owner.LanguageId, value, StringComparison.Ordinal))
                        _owner.SetValue(LanguageIdProperty, value);

                    _owner.JsEval($"if(window.__ph_setLanguage) window.__ph_setLanguage({ToJsString(value)});");
                });
            }
            ...
            public void SetReadOnly(bool isReadOnly)
            {
                RunOnUI(() =>
                {
                    if (_owner.IsReadOnly != isReadOnly)
                        _owner.SetValue(IsReadOnlyProperty, isReadOnly);
        ...
        internal void OnNativeDiagnosticsUpdated(string doc, int[] lines, int[] cols, int[] lens, EditorDiagnosticSeverity[] sev, string[] msgs)
